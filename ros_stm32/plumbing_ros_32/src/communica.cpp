...
bool ROS_READ_FROM_STM32_float(int16_t &p_RPM1, int16_t &p_RPM2, float &p_angle, unsigned char &crtlFlag)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        Stm32ToRos_Angle.tmp_array[i] = recieve_buf[i + 7];
    }
    
    

    //读取控制标志位
    crtlFlag = recieve_buf[11];

    p_RPM1 = Stm32ToRos_RPM1.data;
    p_RPM2 = Stm32ToRos_RPM2.data;
    p_angle = Stm32ToRos_Angle.data;
...
void ROS_WRITE_TO_STM32_FLOAT(short target_RPM1, short target_RPM2, float target_angle, char crtlFlag)
{
    // 协议数据缓存数组
    ...
    RosToStm32_RPM1.data = target_RPM1;
    RosToStm32_RPM2.data = target_RPM2;
    RosToStm32_Float.data = target_angle;
    ...
    for (int i = 0; i < 4; i++)
    {
        Buf[i+7] = RosToStm32_Float.tmp_array[i];
    }
    ...
    Buf[3 + Length + 1] = serial_ender[0];                      //buf[13]
    Buf[3 + Length + 2] = serial_ender[1];                      //buf[14]
